#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_MainMenu

#include "Basic.hpp"

#include "Tyr_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass BP_MainMenu.BP_MainMenu_C
// 0x0008 (0x0350 - 0x0348)
class ABP_MainMenu_C final : public ATyrCommonGameMode
{
public:
	class USceneComponent*                        DefaultSceneRoot;                                  // 0x0348(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, NonTransactional, NoDestructor, HasGetValueTypeHash)

public:
	static class UClass* StaticClass()
	{
		BP_STATIC_CLASS_IMPL("BP_MainMenu_C")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"BP_MainMenu_C")
	}
	static class ABP_MainMenu_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<ABP_MainMenu_C>();
	}
};

}

